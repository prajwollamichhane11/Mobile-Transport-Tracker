...
public class activity2 extends Activity
{
    ...
    int r=0;
    EditText email, busno, password, cpassword, oname, route;
...
//        });
        ...
        route = (EditText)findViewById(R.id.croute);
    ...
    private void addBusdata() {
        ...
        final String bus_no = busno.getText().toString().trim();
        ...
        String add_route = route.getText().toString().trim();

        if ((!TextUtils.isEmpty(owner_name)) && (!TextUtils.isEmpty(bus_no)) && (!TextUtils.isEmpty(add_route)) && (!TextUtils.isEmpty(add_email)) && (!TextUtils.isEmpty(add_password))&& (!TextUtils.isEmpty(add_cpassword)))
        {
            if (add_email.equals("9860"))
            {
                if (cd.isConnected()) {
                    ...
                    databseArtist.addValueEventListener(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            if (r == 0) {
                                ...
                                if (add_cpassword.length() < 6) {
                                    Toast.makeText(getApplicationContext(), "Password must be atleast 6 character long", Toast.LENGTH_SHORT).show();
                                } else {
                                    if (add_password.equals(add_cpassword)) {
                                        String id = databseArtist.push().getKey();


                                        Artist artist = new Artist(bus_no, owner_name, add_email, add_password, lattitude, longitude, id, msg, add_route);
                                        databseArtist.child(id).setValue(artist);

                                        Toast.makeText(getApplicationContext(), "Data added", Toast.LENGTH_SHORT).show();
                                        SharedPrefs.saveSharedSetting(activity2.this, "CaptainCode", "true");
        ...
        if ( r == 1)
        {
            Toast.makeText(getApplicationContext(),"Bus no is already registered",Toast.LENGTH_SHORT).show();
        }
    }

    public void showMessage(String title, String messag)
    {
